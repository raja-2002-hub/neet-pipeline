...
def extract_formulas_from_section(questions, section, client):
    """Extract formulas/principles from extracted questions using Gemini."""
    print(f"  Extracting formulas for {section}...")
    ...
    for q in questions:
        ...
        sol_text = q.get("solution_text", "") or q.get("solution", "")
        if q_text or sol_text:
            context_parts.append(f"Q{q_num}: {q_text}\nSolution: {sol_text}")
            q_ids[q_num] = q.get("question_id", "")
    ...
    if section == "Biology":
        formula_type = "key rules, principles, definitions, and important facts"
        examples = '"All enzymes are proteins but all proteins are not enzymes", "Krebs cycle occurs in mitochondrial matrix"'
    else:
        formula_type = "mathematical formulas, equations, and key relationships"
        examples = '"F = ma" (Newton\'s Second Law), "E = hν" (Planck\'s equation)'

    prompt = f"""
    From these {section} NEET questions and solutions, extract ALL {formula_type} used.

    Questions and solutions:
    {context}

    For EACH formula/principle, return a JSON object:
    {{
        "formula_text": "<the formula or principle, e.g. {examples}>",
        "formula_name": "<name, e.g. Newton's Second Law>",
        "concept": "<one sentence explanation of what it means>",
        "hint": "<memory trick or shortcut to remember>",
        "difficulty": "<Easy/Medium/Hard>",
        "used_in_questions": [<list of question numbers where this formula is used>]
    }}

    CRITICAL: Return ONLY a JSON array. NO markdown. NO explanation.
    Extract at least 5 formulas. Deduplicate — each formula appears only once.
    """
    ...
    try:
        ...
        if isinstance(formulas, list):
            for f in formulas:
                f["section"] = section
                f["topic"] = f.get("topic", "")
                used_in = f.get("used_in_questions", [])
                f["related_question_ids"] = [
                    q_ids.get(qn, f"q{qn}") for qn in used_in if qn in q_ids
                ]
            print(f"    {len(formulas)} formulas extracted for {section}")
            return formulas
    except Exception as e:
        print(f"    Formula extraction error for {section}: {e}")

    return []


def extract_and_load_formulas(all_questions, paper_id, client):
    """Extract formulas from all sections and load to BigQuery."""
    print("\n── Formula Extraction ──")

    all_formulas = []
    seen_texts = {}

    for section in ["Physics", "Chemistry", "Biology"]:
        formulas = extract_formulas_from_section(all_questions, section, client)

        for f in formulas:
            ft = f.get("formula_text", "").strip()
            if not ft:
                continue

            ft_lower = ft.lower()
            if ft_lower in seen_texts:
                existing = seen_texts[ft_lower]
                existing_ids = existing.get("related_question_ids", [])
                new_ids = f.get("related_question_ids", [])
                merged = list(set(existing_ids + new_ids))
                existing["related_question_ids"] = merged
                continue

            formula_id = f"{paper_id}_formula_{len(all_formulas) + 1}"
            formula_row = {
                ...
                "topic": f.get("topic", ""),
                "difficulty": normalise_difficulty(f.get("difficulty", "Medium")),
                "hint": f.get("hint", ""),
                "related_question_ids": json.dumps(f.get("related_question_ids", [])),
                "created_at": datetime.utcnow().isoformat(),
            }
            all_formulas.append(formula_row)
            seen_texts[ft_lower] = formula_row

    if not all_formulas:
        print("  No formulas extracted")
        return 0

    print(f"  Loading {len(all_formulas)} formulas to BigQuery...")
    bq = bigquery.Client(project=PROJECT)
    from google.cloud.bigquery import LoadJobConfig, SourceFormat

    try:
        jc = LoadJobConfig()
        jc.source_format = SourceFormat.NEWLINE_DELIMITED_JSON
        jc.write_disposition = "WRITE_APPEND"
        jc.autodetect = False

        data = "\n".join(json.dumps(r) for r in all_formulas).encode()
        job = bq.load_table_from_file(
            io.BytesIO(data),
            f"{PROJECT}.{DATASET}.{FORMULAS_TABLE}",
            job_config=jc
        )
        job.result()
        print(f"  ✓ {len(all_formulas)} formulas loaded")
        return len(all_formulas)
    except Exception as e:
...
def process_pdf(cloud_event):
    """
    Pipeline v5 — FINAL + Formula Extraction:
      ...
      Formulas:  Extracted from all sections after question loading
    """
    ...
    extract_and_load_formulas(all_q, paper_id, client)  # ← NEW: Formula extraction
    run_dbt_transformations(paper_id)
    print(f"\n✓ Complete: {len(all_q)} questions + formulas from {file_name}")