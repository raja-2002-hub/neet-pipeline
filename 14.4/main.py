...
def normalize_sections(sections):
    """Handles Gemini returning sections as dicts or strings."""
    result = []
    for sec in sections:
...
def process_pdf(cloud_event):
    ...
    Pipeline:
      ...
      6.   Renumber
      7.   Extract diagrams (HYBRID):
           → ALWAYS run PDF extractor (PyMuPDF) — works for Physics
           → If DOCX exists: ALSO run DOCX extractor for Chemistry+Biology
             and override PDF results for those sections
           → If no DOCX: PDF results used for all sections
      8.   Attach URLs
    ...
    # ══════════════════════════════════════════════════════════
    # Step 7: HYBRID diagram extraction
    #
    # ALWAYS run PDF extractor first — Physics works perfectly
    # with "Question No." markers and PyMuPDF raster extraction.
    #
    # Then IF a DOCX exists, run DOCX extractor for Chemistry
    # and Biology only, and override the PDF results for those
    # sections (because PDF has broken zone mapping for Chemistry
    # where iLovePDF stripped all "Question No." markers).
    # ══════════════════════════════════════════════════════════

    url_map = {}

    # Step 7A: PDF extractor (PyMuPDF) — handles Physics perfectly
    try:
        print("\n── Step 7A: PDF extraction (Physics) ──")
    ...
    try:
        ...
        if docx_blob.exists():
            ...
            # Override PDF results for Chemistry and Biology ONLY
            # Physics stays from PDF extractor (which works perfectly)
            overridden = 0
            for key, zones in docx_url_map.items():
                if key.startswith("Chemistry_") or key.startswith("Biology_"):
                    url_map[key] = zones
                    overridden += 1
            print(f"  DOCX overrode {overridden} Chemistry+Biology entries")
        else:
            print(f"\n  No DOCX found ({docx_name}) — PDF results for all sections")
    except Exception as e:
