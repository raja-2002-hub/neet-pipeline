...
def process_pdf(cloud_event):
    """
    Pipeline:
      1-3. Metadata, lock, read PDF
      4.   Detect pattern (Gemini)
      5.   Extract questions (Gemini reads PDF)
      6.   Renumber
      7.   Extract diagrams (HYBRID — PER-ZONE MERGE):
           → ALWAYS run PDF extractor (PyMuPDF) — good for raster images
           → If DOCX exists: run DOCX extractor for Chemistry+Biology
           → PER-ZONE MERGE: for each question, for each zone:
             - If DOCX has images → use DOCX (better for vector structures)
             - If DOCX empty but PDF has images → use PDF (raster fallback)
           → Physics stays 100% from PDF
      8.   Attach URLs
      9-11. Save JSON, BigQuery, dbt
    """
    ...
    # ══════════════════════════════════════════════════════════
    # Step 7: HYBRID diagram extraction — PER-ZONE MERGE
    #
    # PDF (PyMuPDF) = good at raster images with page positions
    #   → question diagrams, solution diagrams, page-level crops
    #
    # DOCX (python-docx) = good at vector chemical structures
    #   → option-level structures with element-order zone mapping
    #
    # Per-zone merge: for each Chemistry/Biology question,
    # take the BEST image source for each zone:
    #   - DOCX has images for zone → use DOCX
    #   - DOCX empty, PDF has images → use PDF
    #   - Neither has images → zone stays empty
    # ══════════════════════════════════════════════════════════

    url_map = {}
    pdf_url_map = {}

    # Step 7A: PDF extractor (PyMuPDF) — handles all sections
    try:
        print("\n── Step 7A: PDF extraction (all sections) ──")
        pdf_url_map = extract_diagrams(pdf_bytes, paper_id,
                                       corrected_questions=all_q)
        url_map.update(pdf_url_map)
        physics_count = sum(1 for k in pdf_url_map if k.startswith("Physics_"))
        chem_count = sum(1 for k in pdf_url_map if k.startswith("Chemistry_"))
        bio_count = sum(1 for k in pdf_url_map if k.startswith("Biology_"))
        print(f"  PDF results: Physics={physics_count}, "
              f"Chemistry={chem_count}, Biology={bio_count}")
    except Exception as e:
        print(f"  PDF extraction failed: {e}")

    # Step 7B: DOCX extractor + PER-ZONE MERGE
    docx_name = file_name.replace(".pdf", ".docx").replace(".PDF", ".docx")
    try:
        docx_blob = sc.bucket(bucket_name).blob(docx_name)
        if docx_blob.exists():
            print(f"\n── Step 7B: DOCX extraction + per-zone merge ──")
            ...
            # PER-ZONE MERGE
            #
            # For each Chemistry/Biology question:
            #   - DOCX wins where it has images (vector)
            #   - PDF fills empty zones (raster fallback)
            # Physics stays 100% from PDF (untouched)
            # ─────────────────────────────────────────────

            ALL_ZONES = ["question", "option_1", "option_2",
            ...
            # Collect all Chemistry/Biology keys from both sources
            all_keys = set()
            for k in docx_url_map:
                if k.startswith("Chemistry_") or k.startswith("Biology_"):
                    all_keys.add(k)
            for k in url_map:
                if k.startswith("Chemistry_") or k.startswith("Biology_"):
                    all_keys.add(k)
            ...
            for key in sorted(all_keys):
                docx_zones = docx_url_map.get(key, {})
                pdf_zones = pdf_url_map.get(key, {})
                merged = {}

                for zone in ALL_ZONES:
                    docx_urls = docx_zones.get(zone, [])
                    pdf_urls = pdf_zones.get(zone, [])

                    if docx_urls:
                        merged[zone] = docx_urls
                        docx_zones_used += 1
                    elif pdf_urls:
                        merged[zone] = pdf_urls
                        pdf_zones_used += 1
            ...
            print(f"\n  ── Per-zone merge results ──")
            print(f"  Questions merged: {merged_count}")
            print(f"  Zones from DOCX: {docx_zones_used} (vector)")
            print(f"  Zones from PDF:  {pdf_zones_used} (raster)")

            for key in sorted(all_keys):
                q_num = key.split("_")[-1]
                sec = key.split("_")[0]
                zones = url_map.get(key, {})
                d_z = docx_url_map.get(key, {})
                srcs = []
                for z in ALL_ZONES:
                    if z in zones:
                        src = "DOCX" if z in d_z and d_z[z] else "PDF"
                        srcs.append(f"{z}={src}")
                if srcs:
                    print(f"  {sec} Q{q_num}: {', '.join(srcs)}")

        else:
            print(f"\n  No DOCX found ({docx_name}) — PDF for all")
    except Exception as e:
        print(f"  DOCX extraction failed: {e} — PDF results remain")
