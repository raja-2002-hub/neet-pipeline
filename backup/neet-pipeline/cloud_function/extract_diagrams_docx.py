"""
extract_diagrams_docx.py — DOCX diagram extraction v4

TWO-PASS ELEMENT-ORDER APPROACH
================================
Proven on actual 2016 NEET DOCX (iLovePDF conversion).

Root cause of all previous failures: the old code read paragraph TEXT
to detect zones, then assigned ALL images in that paragraph to the
detected zone. But iLovePDF merges option markers and images into
single paragraphs in inconsistent order:

  P1502: IMG:image70 > IMG:image71 > TXT:(2) > IMG:image72
  
The old code saw "(2)" in the text → assigned ALL 3 images to option_2.
But image70 and image71 appear BEFORE "(2)" → they should be option_1.

FIX: Walk XML elements within each paragraph in document order.
Track zone changes as text elements are encountered. Assign each
image to the zone that was active AT THAT POINT in the element stream.

ADDITIONAL FIXES:
  1. Implicit option_1: when (1) marker is missing but images appear
     after question text and before (2), they're option_1.
  2. Sol. paragraph images: ALL images in a paragraph containing Sol.
     are solution images (iLovePDF places them before the Sol. text).
  3. Redistribution: when N images sit in question zone and N empty
     option slots exist, distribute 1 image per option (Q152 pattern).
  4. Junk image filter: pixel-density check for empty rectangles/dots.
  5. Chemistry+Biology only: Physics handled by PDF extractor.
"""
...
def is_junk_image(image_bytes):
    ...
    try:
        ...
        if dark / total < JUNK_MIN_DARK_RATIO:
            return True
        # Empty rectangle: border only, white interior
...
def para_has_sol(para):
    """Check if paragraph contains Sol. marker anywhere in its XML."""
...
def extract_diagrams_docx(docx_bytes, paper_id, corrected_questions=None):
    """
    Two-pass element-order extraction for Chemistry+Biology.
    Physics is skipped (PDF extractor handles it).
    """
    print(f"\n{'='*60}")
    print(f"DOCX EXTRACTION v4 (element-order) — {paper_id}")
    print(f"{'='*60}")
    ...
    try:
        ...
        sol_positions = []
        section_ranges = {}  # section → (start_para, end_para)
        ...
        for sec, (s, e) in section_ranges.items():
            tag = "→ DOCX" if sec in DOCX_SECTIONS else "→ SKIP"
        ...
        # ─── PASS 2: For each section we handle, walk questions ───
        ...
        for section in DOCX_SECTIONS:
            ...
            print(f"\n  Processing {section}: {len(sec_sols)} Sol., "
                  f"{len(sec_qs)} questions")
            ...
            for qi in range(min(len(sec_sols), len(sec_qs))):
                ...
                # ── Element-order walk within this question ──

                current_zone = "question"
                assignments = []  # (rid, zone)
                seen_option = False
                ...
                for pi in range(q_start, q_end + 1):
                    ...
                    # FIX 2: Sol. paragraph — ALL images are solution
                    ...
                    # Normal paragraph — walk elements in order
                    for child in para._element.iter():
                        if child.tag.endswith('}t'):
                            ...
                            # Option markers change zone immediately
                            ...
                            # Track question text (for implicit option_1)
        ...
        for sec in DOCX_SECTIONS:
            ...
            if keys:
                ...
                print(f"  {sec}: {len(keys)} questions "
